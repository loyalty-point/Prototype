...
// add or update into admin_registration
$query = "select username from admin_registration where username='".$username."'";
$query_exec = mysqli_query($localhost, $query);
$row = mysqli_fetch_array($query_exec);
$usernameTest = $row['username'];

if($usernameTest == "") { // Chưa có thông tin đăng kí, insert vào
	$query = "insert into admin_registration values ('"
							.$username."','"
							.$regID."')";  
	...
	$query = "update admin_registration set regID = '" . $regID . "' where username = '" . $username . "'";
